/*
	...
	Description: 	In the 20×20 grid below, four numbers along a diagonal line have been marked in red.
					...
					000: 08 02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08
					020: 49 49 99 40 17 81 18 57 60 87 17 40 98 43 69 48 04 56 62 00
					040: 81 49 31 73 55 79 14 29 93 71 40 67 53 88 30 03 49 13 36 65
					060: 52 70 95 23 04 60 11 42 69 24 68 56 01 32 56 71 37 02 36 91
					080: 22 31 16 71 51 67 63 89 41 92 36 54 22 40 40 28 66 33 13 80
					...
					320: 04 42 16 73 38 25 39 11 24 94 72 18 08 46 29 32 40 62 76 36	<----- 40 is GRID_SIZE - 3 - (20*3), the top left of our 4x4 window
...
void ReadData();
void FindGreatestProduct();

const int COLUMN_SIZE = 20;
const int ROW_SIZE = 20;
const int GRID_SIZE = COLUMN_SIZE*ROW_SIZE;
...
void ReadData()
{
	...
	inputFile.open("euler11data.txt");
...
void FindGreatestProduct()
{
	...
	int largest {};
	int value {};
	while(i <= (GRID_SIZE-3-(ROW_SIZE*3)))
	{
		// Within our 4x4 window, we need to check...
		// Product of each row
		for(int j = 0; j < 4; ++j)
		{
			int rowOffset = j*ROW_SIZE;
			value = Grid[i+rowOffset+0] * Grid[i+rowOffset+1] * Grid[i+rowOffset+2] * Grid[i+rowOffset+3];
			if(value > largest) largest = value;
		}
		// Product of each column
		for(int j = 0; j < 4; ++j)
		{
			value = Grid[i+(ROW_SIZE*0)+j] * Grid[i+(ROW_SIZE*1)+j] * Grid[i+(ROW_SIZE*2)+j] * Grid[i+(ROW_SIZE*3)+j];
			if(value > largest) largest = value;
		}
		// Product of diagonals
		value = Grid[i+(ROW_SIZE*0)+0] * Grid[i+(ROW_SIZE*1)+1] * Grid[i+(ROW_SIZE*2)+2] * Grid[i+(ROW_SIZE*3)+3];
		if(value > largest) largest = value;
		value = Grid[i+(ROW_SIZE*0)+3] * Grid[i+(ROW_SIZE*1)+2] * Grid[i+(ROW_SIZE*2)+1] * Grid[i+(ROW_SIZE*3)+0];
		if(value > largest) largest = value;
